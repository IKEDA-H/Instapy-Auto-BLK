...
session = InstaPy(username = "bluelemonkitchen", password = "bluelemon1022", headless_browser=False)
...
with smart_run(session):
    ...
    #2_コメントの内容設定(10種類)
    #session.set_comments([u'♡♡♡♡ @{}', u'💛💛💛 @{}', u'👏👏 @{}', u'👍👍 @{}', u'😍😍😍 @{}', u'😄😄😄 @{}', u'💘 @{}'])
    ...
    session.set_quota_supervisor(enabled=False, sleep_after=["likes", "comments_d", "follows", "unfollows", "server_calls_h"], sleepyhead=True, stochastic_flow=True, notify_me=True,
                              peak_likes_hourly=70, peak_likes_daily=650, peak_comments_hourly=5, peak_comments_daily=None,
                                peak_follows_hourly=None, peak_follows_daily=None, peak_unfollows_hourly=None, peak_unfollows_daily=None, peak_server_calls_hourly=1000, peak_server_calls_daily=4000)
    ...
    session.set_action_delays(enabled=True, like=2, comment=2, follow=2, unfollow=28, story=2, randomize=True, random_range_from=70, random_range_to=120, safety_match=True)
    ...
    session.like_by_tags(tags=['料理好きな人と繋がりたい','ひとりごはん'], use_random_tags=True, amount=5, skip_top_posts=True, randomize=False, media=None, interact=True)
